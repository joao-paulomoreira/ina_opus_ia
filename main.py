...
def retorna_resposta_modelo(mensagens, openai_key, modelo='gpt-4', temperatura=0, stream=True, max_tokens=500):
    ...
    if stream:
        response_stream = openai.completions.create(  # Alteração aqui para 'completions.create'
            model=modelo,
            messages=mensagens,
        ...
        response = openai.completions.create(  # Alteração aqui também para 'completions.create'
            model=modelo,
            messages=mensagens,
            temperature=temperatura,
            max_tokens=max_tokens
...
Eventos adversos: Sempre explique como "Erro com dano / Error with damage", sem variações e nunca no final da frase.
